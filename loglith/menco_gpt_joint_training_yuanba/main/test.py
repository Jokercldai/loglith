...
#     """
#     ⭐ 将原始测井曲线 (B, T, C) 和 mask (B, T, C)
#        转换为 GPT 模型使用的 chunk 表示：

#        - x_chunk:  (B, seq_len, token_dim=chunk_size*C)
#        - tgt_chunk:(B, seq_len, C)      每个 chunk 内 mask 加权平均
#        - mask_chunk: (B, seq_len, C)    每个 chunk+曲线维度是否有有效点
#     """
#     B, T, C = xb_ori.shape
#     chunk_size = args.chunk_size         # 一般为 8
#     token_dim = len(args.fea)*chunk_size     # C * chunk_size  #model.token_dim   
#     seq_len = T // chunk_size             # 一般为 512//8=64

#     # ⭐ 安全裁剪，确保能整除
#     T_use = seq_len * chunk_size
#     xb_ori = xb_ori[:, :T_use, :]         # (B, T_use, C)
#     masko  = masko[:, :T_use, :]          # (B, T_use, C)

#     # (B, T_use, C) → (B, seq_len, chunk_size, C)
#     xb_chunks   = xb_ori.view(B, seq_len, chunk_size, C)
#     mask_chunks = masko.view(B, seq_len, chunk_size, C)

#     # ⭐ 输入给 GPT 的 token：flatten chunk 维度 → (B, seq_len, chunk_size*C)
#     x_chunk = xb_chunks.reshape(B, seq_len, token_dim)

#     # # ⭐ 构造标签：对每个 chunk 内做 mask 加权平均 → (B, seq_len, C)
#     # #   sum_vals: 有效点的加权和
#     # sum_vals = (xb_chunks * mask_chunks).sum(dim=2)          # (B, seq_len, C)
#     # valid_cnt = mask_chunks.sum(dim=2)                       # (B, seq_len, C)

#     # eps = 1e-8
#     # tgt_chunk = sum_vals / (valid_cnt + eps)                 # (B, seq_len, C)

#     # # ⭐ 如果某个 (chunk, feature) 上所有点都是 NaN → valid_cnt=0 → mask_chunk=0
#     # mask_chunk = (valid_cnt > 0).float()                     # (B, seq_len, C)

#     return x_chunk, #tgt_chunk, mask_chunk




def test_one_epoch(model, test_loader, criterion, device, num_class, args):
    model.eval()
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            batch_x, target, fea_ori, masko = data['fea_log'], data['lith'], data['fea_ori'], data['masko']
            sam_md = data['sam_md']


            x_chunk, mask_tokens = chunkify_tokens(
                batch_x, masko, args
            )  # x_chunk:(B,64,40), tgt_chunk/mask_chunk:(B,64,5)
            
            if args.model_type in ["LogBERT", "LogBERT_parttune"]:
                print("====== 使用 LogBERT 进行测试")
                outputs = model.finetune_step(x_chunk, mask_tokens)
            else:
                outputs, recon = model(x_chunk, model_task=args.stage)
    ...
    f1_macro = f1_score(y_true, y_pred, average="macro")  # 整体 F1   macro; weighted
...
if __name__ == "__main__":
    ...
    if args.stage in ["pretrain", "finetune"]:
        if args.model_type in ["LogGPT_Chunk16", "LogGPT_Chunk512_linshi", 
                               "LogGPT_Chunk32", "LogGPT_Chunk1"]:
            print(f"====== 使用 {args.model_type} 模型测试")
            model = LogGPT_Chunk16(args).to(device)
        elif args.model_type in ["LogBERT", "LogBERT_parttune"]:
            print("====== 使用 LogBERT 模型测试")
        ...
        if args.model_type in ["LogGPT_Chunk16", "LogGPT_Chunk32", "LogGPT_Chunk1"]:
            print(f"====== 使用 {args.model_type}（无预训练）")
            model = LogGPT_Chunk16(args).to(device)
    ...
    # Load model
    model_name = "2.7_best_model_epo_5746_0p3505.pth" #"" #"best_model.pth"  #"11.7_best_model_exp12_3144epo.pth"  #'11.29_best_model_exp19_3985epo.pth'
    ...
    for id in range(len(all_well_name)):
        ...
        test_loss, test_acc, test_per_class_acc, preds, targets, fea_oris, f1_macro, per_class_f1, sam_mds = \
            test_one_epoch(model, test_loader, criterion, device, 
